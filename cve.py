...
from generic_storage import GenericStorageHelper
...
class Cve:
    def __init__(self):
        ...
        # Add Config for Storage to Disk
        self.root_storage_location = get_config_variable(
            "CONNECTOR_ROOT_STORAGE_LOCATION", ["connector", "root_storage_location"], config, False
        )
        self.root_queue_location = get_config_variable(
            "CONNECTOR_ROOT_QUEUE_LOCATION", ["connector", "root_queue_location"], config, False
        )
        self.dir = get_config_variable(
            "CVE_DIR", ["cve", "dir"], config, False
        )
        self.store_or_read = get_config_variable(
            "CONNECTOR_STORE_OR_READ", ["connector", "store_or_read"], config, False
        )
        self.storage_helper = GenericStorageHelper(self.root_storage_location, self.root_queue_location, self.dir)
    ...
    def convert_and_send(self, url, work_id):
        try:
            ...
            # Store copy of raw data if required
            if (self.store_or_read.upper() == "STORE"):
                store_filename = "CVE" + datetime.now().strftime("%Y-%m-%d_%H%M%S") + ".json.gz"
                saved_filename = self.storage_helper.store(store_filename, image)
                self.helper.log_info(f"Saved raw data in file {saved_filename}")
            ...
            # Read from Disk if required
            if (self.store_or_read.upper() == "READ"):
                if (self.root_queue_location is not None
                    and len(self.root_queue_location) > 0 ):
                    
                    while (self.storage_helper.does_next_file_exist()):
                        # Unzipping file
                        self.helper.log_info("Unzipping the file")
                        file_ref = self.storage_helper.get_next_file_ref()
                        with gzip.open(file_ref, "rb") as f_in:
                            with open("data.json", "wb") as f_out:
                                shutil.copyfileobj(f_in, f_out)
                        # converting the file to stix2
                        self.helper.log_info("Converting the file")
